...
export class MapManager {
  ...
  init() {
    ...
    const sky = new THREE.Mesh(
      new THREE.SphereGeometry(300, 28, 20),
      new THREE.MeshBasicMaterial({ color: 0xaecbe5, side: THREE.BackSide })
    );
    this.scene.add(sky);
  ...
  createGroundMaterial() {
    ...
    ctx.fillStyle = '#55735e';
    ctx.fillRect(0, 0, 256, 256);
    for (let i = 0; i < 1200; i++) {
      const x = Math.random() * 256;
      const y = Math.random() * 256;
      const shade = 65 + Math.random() * 35;
      ctx.fillStyle = `rgba(${shade}, ${90 + Math.random() * 40}, ${shade}, 0.18)`;
  ...
  spawnDust() {
    ...
    const leafPos = new Float32Array(260 * 3);
    for (let i = 0; i < 260; i++) {
      leafPos[i * 3] = (Math.random() - 0.5) * 130;
      leafPos[i * 3 + 1] = Math.random() * 10 + 1;
      leafPos[i * 3 + 2] = (Math.random() - 0.5) * 130;
    }
    const leafGeo = new THREE.BufferGeometry();
    leafGeo.setAttribute('position', new THREE.BufferAttribute(leafPos, 3));
    this.leaves = new THREE.Points(
      leafGeo,
      new THREE.PointsMaterial({ color: 0xd1a56a, size: 0.14, opacity: 0.36, transparent: true, depthWrite: false })
    );
  ...
  update(time, rainy = false) {
    ...
    if (this.leaves) {
      ...
      for (let i = 0; i < arr.length; i += 3) {
        arr[i] += Math.sin(time * 0.0005 + arr[i + 2]) * 0.002;
        arr[i + 1] -= 0.006;
        arr[i + 2] += Math.cos(time * 0.0004 + arr[i]) * 0.0015;
        if (arr[i + 1] < 0.3) arr[i + 1] = 8 + Math.random() * 4;
      }
      this.leaves.geometry.attributes.position.needsUpdate = true;
      this.leaves.material.opacity = rainy ? 0.24 : 0.36;
    }
  }
