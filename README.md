...
- **GUI/HUD improvements**
  ...
  - Live nearest-target hint with distance for faster routing
...
- **Systems retained**
  ...
  - Interaction system refactored with cached mesh lookup for lower per-frame overhead
...
The deployment build failure (`npm run build exited with 1`) is typically caused by CI environment mismatches for build tooling.
This project now includes deployment-safe defaults:

- `vercel.json` with explicit:
  - `buildCommand: npm run build`
  - `outputDirectory: dist`
  - SPA rewrite to `index.html`
- `vite.config.js` with explicit `outDir: dist`
- Node/npm engine pinning in `package.json` (`node >= 20.10`, `npm >= 10`)
- Server-only code excluded from static deployment via `.vercelignore`

If deploying only the web client on Vercel, this setup is ready.
